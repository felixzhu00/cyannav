...
function MapViewingPage() {
  ...
  useEffect(() => {
    ...
    if (store.currentMap && store.currentMap.commentsId) {
      const fetchComments = async () => {
        const commentsPromises = store.currentMap.commentsId.map((commentId) =>
          store.getCommentById(commentId)
        );
        try {
          const commentsResponses = await Promise.all(commentsPromises);
          const fetchedComments = commentsResponses.map((response) => ({
            ...response,
            hasLikedComment: response.upvotes.includes(auth.user.userId),
            hasDislikedComment: response.downvotes.includes(auth.user.userId),
          }));
    ...
    setComments(
      comments.map((comment) => {
        if (comment.id === commentId) {
          ...
          const hasDisliked = comment.downvotes.includes(auth.user.userId);
          return {
            ...comment,
            upvotes: hasLiked
              ? comment.upvotes.filter((userId) => userId !== auth.user.userId)
        ...
        >
          ...
          {comments.map((comment, index) => (
            <Paper key={index} sx={commentBubbleStyle}>
              ...
              <IconButton
                sx={{
                  color: comment.upvotes.includes(auth.user.userId)
                    ? "black"
                    : "default",
                }}
              ...
              <IconButton
                sx={{
                  color: comment.downvotes.includes(auth.user.userId)
                    ? "black"
                    : "default",
                }}
                onClick={() => handleDislikeComment(comment.id)}
              >
                <ThumbDown sx={{ fontSize: "small" }}></ThumbDown>
              </IconButton>
              <Typography variant="caption">
            ...
            >
              ...
              >
                <AccordionSummary expandIcon={<ExpandMore />}>
                  <Typography>Advanced Editing Features</Typography>
                </AccordionSummary>
                ...
                {features[store.currentArea]?.fields?.immutable &&
                  ["radius", "scale", "weight", "color"].map((key) => {
                    ...
                    const keyExists =
                      features[store.currentArea]?.fields?.immutable &&
                      features[store.currentArea].fields.immutable[key] !==
                        undefined &&
                      features[store.currentArea].fields.immutable[key] !==
                        null;

                    // Check if the key has a non-null value based on the map type
                    const mapType = store.currentMap.mapType; // Replace with the actual way you determine the map type

                    return (
                      keyExists &&
                      mapTypes[key]?.includes(mapType) && (
                        <Box
                          key={key}
                          sx={{
                            display: "flex",
                ...
                <AccordionDetails>
                  ...
                  >
                    <ListItem
                      aria-expanded={handleChoroplethClick ? "true" : undefined}
                  ...
                  >
                    {store.currentArea !== null &&
                      ...
                      Object.entries(
                        ...
                        if (isNumeric(value) && key != "byFeature") {
                          return (
                            <MenuItem
                              key={key}
                              onClick={() => {
                                handleSelectedByFeature(key);
                                setSelectedItem(key);
                              }}
                            >
                              {key}
  ...
  return (
    ...
    >
      ...
      <Box sx={{ gridColumn: "1", gridRow: "2", zIndex: 0 }}>{mapView()}</Box>
